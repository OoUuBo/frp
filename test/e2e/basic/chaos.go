...
var _ = Describe("[Feature: Chaos]", func() {
	...
	It("reconnect after frps restart", func() {
		...
		ps, _, err := f.RunFrps([]string{"-c", serverConfigPath})
		framework.ExpectNoError(err)

		pc, _, err := f.RunFrpc([]string{"-c", clientConfigPath})
		framework.ExpectNoError(err)
		...
		_, _, err = f.RunFrps([]string{"-c", serverConfigPath})
		framework.ExpectNoError(err)
		...
		_, _, err = f.RunFrpc([]string{"-c", clientConfigPath})
		framework.ExpectNoError(err)
